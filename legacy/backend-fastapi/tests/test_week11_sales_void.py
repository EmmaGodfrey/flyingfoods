"""Week 11 tests for sales void workflow contracts."""
...
def _create_sale(client: TestClient) -> int:
    response = client.post(
        ...
        headers={"x-request-id": "req-week11-sale"},
    )
    assert response.status_code == 201
    return int(response.json()["sale_id"])


def test_void_sale_reverses_stock_emits_audit_and_invalidates_cache() -> None:
    ...
    try:
        ...
        void_response = client.post(
            ...
            json={"reason": "cashier mistake"},
            headers={"x-request-id": "req-week11-void"},
        )
        ...
        async def _verify() -> tuple[Sale | None, float, float, AuditLog | None]:
            async with session_factory() as session:
                ...
                audit_row = (
                    await session.execute(
                        select(AuditLog).where(AuditLog.entity_type == "sale", AuditLog.entity_id == str(sale_id), AuditLog.action == "sale.void")
                    )
                ).scalar_one_or_none()
...
def test_void_sale_denies_cross_branch_access() -> None:
    session_factory = _build_test_db()
    _seed_pos_sale_data(session_factory)
    ...
    try:
        client = TestClient(app)
        response = client.post(f"/sales/{sale_id}/void", json={"reason": "x"})
...
def test_void_sale_rejects_duplicate_void() -> None:
    session_factory = _build_test_db()
    _seed_pos_sale_data(session_factory)
    ...
    try:
        ...
        first = client.post(f"/sales/{sale_id}/void", json={"reason": "mistake"})
        assert first.status_code == 200

        second = client.post(f"/sales/{sale_id}/void", json={"reason": "mistake"})
        assert second.status_code == 400
        assert second.json()["detail"] == "Sale already voided"
    finally:
        app.dependency_overrides.clear()
